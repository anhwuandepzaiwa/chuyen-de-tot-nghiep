async function getProducts() {
    ...
    try {
        ...
        const response = await fetch("http://localhost:8080/api/getProductFromUser?category=Trang điểm&page=1&limit=8", requestOptions);
        ...
        if (jsonResponse && Array.isArray(jsonResponse.data)) {
            ...
            if (categoryElement) {
                categoryElement.textContent = jsonResponse.category || "Trang điểm"; // Replace with actual category name if available
                categoryElement.setAttribute('data-category', jsonResponse.category || "Trang điểm"); // Set the category data attribute
            }
